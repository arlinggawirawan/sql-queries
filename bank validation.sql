select a.user_id, f.name, a.bank_account_holder_name, a.bank_name,			
-- f.mobile, f.email, f.gender, d.monthly_salary, e.education, c.age_group, 			
a.created_date register_date, case when applied=1 then 'Applied' else 'Not Applied' end as apply_flag			
-- a.province 			
from			
    ...
    upa.province, fub.bank_account_holder_name, fub.bank_name			
	from (		
	...
	left join (		
		select distinct user_id, bank_account_holder_name, bank_name from (	
			(select distinct user_id, bank_account_holder_name, bank_name
			from finance.finance_user_bankcard -- where is_used=2
            )			
			union all
			(select distinct user_id, '' as bank_account_holder_name, '' as bank_name
	...
	left join (		
		...
		left join indo_regions ir on (c.province_id = ir.province_id and c.city_id = ir.city_id and c.area_id = ir.area_id and c.village_id = ir.village_id)	
	...
	where date(ui.created_at) >= '2019-12-01'		
)a			
left join			
...
left join			
	(select a.user_id, case when timestampdiff(year,birthday,date(created_at)) between 21 and 25 then 'a. 21 - 25'		
